...
# Copied from https://github.com/PagerDuty/pd-oncall-chat-topic/blob/master/lambda/main.py
...
def get_user(schedule_id):
    ...
    if normal.status_code == 404:
        logger.critical(
            "ABORT: Not a valid schedule: {}".format(schedule_id))
        return False
    try:
        ...
        override = requests.get(
            override_url, headers=headers, params=payload)
...
def get_slack_topic(channel):
    ...
    try:
        r = requests.get(
            'https://slack.com/api/conversations.info', params=payload)
...
def update_slack_topic(channel, proposed_update):
    ...
    if current_full_topic:
        ...
        # The original intent was to preserve a '|' in the schedule name but
    ...
    if proposed_update != first_part:
        ...
        if len(topic) > 250:
            topic = topic[0:247] + "..."
        payload['topic'] = topic
        logger.debug("New Topic: '{}'".format(topic))
        r = requests.post(
...
def do_work(obj):
    ...
    schedule = figure_out_schedule(obj['schedule'])
    if schedule:
